...
=head1 .nes.cfg configuration files
...
=head2 Perl syntax in the value
...
Is a class for a single access interface to Nes. Return the new object
or instance of this if is created previously.
...
Remenber, Nes is oriented at HTML and not the Perl code.
...
If we make a variable 'yes_no' will not get run code that contains the
variable. For safety, you can not execute code from a Nes variable. 
The effect of 'no_nes' is that the tags are visible, and 'yes_no' 
is disappearing.
